...
class Project {
  ...
  static async create(data) {
    ...
    const result = await db.run(
      `INSERT INTO projects (name, client, contract_sum, location, start_date, end_date, status, project_manager, description)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [data.name, data.client, data.contract_sum, data.location, data.start_date, data.end_date, data.status, data.project_manager, data.description]
    );
